  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-xl md:max-w-2xl">
        {/* Modal Header with pr-12 to guarantee NO collision with the X close button */}
        <DialogHeader className="shrink-0 px-6 pt-5 pb-4 border-b border-border/70 text-left pr-12">
              ...
              <div>
                ...
                <DialogDescription className="text-xs text-muted-foreground mt-0.5">
                  {selectedGarments.length > 1 ? (
                    <span>
                      Calculating sizes for <strong className="text-foreground">{selectedGarments.length} selected garments</strong> (Top & Bottom)
                    </span>
                  ) : selectedGarments[0] ? (
                    <span>
          ...
          {/* Clean Full-Width Step Progress Indicator */}
          <div className="flex items-center gap-3 pt-3">
            ...
            <span className="text-[11px] font-semibold text-muted-foreground shrink-0">
              {step === 1 ? "1. Inputs" : step === 2 ? "2. AI Calculate" : "3. Sizing Result"}
            </span>
          </div>
        </DialogHeader>
