        ...
        {PRESET_PROFILES.map((preset) => {
          ...
          return (
            <button
              ...
              className={`p-2.5 sm:p-3 rounded-xl border text-left transition-all cursor-pointer flex flex-col justify-between gap-1.5 relative overflow-hidden group ${
                isSelected
            ...
            >
              <div className="flex items-center justify-between w-full">
                ...
                {isSelected && (
                  <Check className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-background font-bold shrink-0" />
                )}
              </div>

              <div>
                <p className="text-[11px] sm:text-xs font-semibold leading-tight">{preset.label}</p>
                <p
